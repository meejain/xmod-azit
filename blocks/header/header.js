...
export default async function decorate(block) {
  ...
  const navChildren = [...nav.children].filter((child) => child.tagName === 'DIV');
  classes.forEach((c, i) => {
  ...
  // Search trigger — create programmatically (DA strips icon spans)
  let searchTriggerP = navTools?.querySelector('.icon-search')?.closest('p');
  if (!searchTriggerP && navTools) {
    // DA doesn't have the icon-search span — create search trigger
    searchTriggerP = document.createElement('p');
    searchTriggerP.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>';
    const toolsWrapper = navTools.querySelector('.default-content-wrapper') || navTools;
    toolsWrapper.append(searchTriggerP);
  }
  if (searchTriggerP) {
